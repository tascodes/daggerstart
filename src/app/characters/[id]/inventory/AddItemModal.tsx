...
export default function AddItemModal({
  ...
  const allItems = [
    ...
    ...Items.map(item => ({ ...item, type: "ITEM" as ItemType })).sort((a, b) => a.name.localeCompare(b.name)),
    ...Consumables.map(consumable => ({ ...consumable, type: "CONSUMABLE" as ItemType })).sort((a, b) => a.name.localeCompare(b.name)),
    // Armor and weapons sorted by tier, then alphabetically
    ...Armors.map(armor => ({ ...armor, type: "ARMOR" as ItemType })).sort((a, b) => {
      if (a.tier !== b.tier) return a.tier - b.tier;
      return a.name.localeCompare(b.name);
    }),
    ...Weapons.map(weapon => ({ ...weapon, type: "WEAPON" as ItemType })).sort((a, b) => {
      if (a.tier !== b.tier) return a.tier - b.tier;
      return a.name.localeCompare(b.name);
    }),
  ];
  ...
  const filteredItems = allItems.filter((item) => {
    ...
    if (filterTier !== "ALL" && (item.type === "ARMOR" || item.type === "WEAPON")) {
  ...
  const renderItemDetails = (item: any) => {
    switch (item.type) {
      case "ITEM":
      case "CONSUMABLE":
        const itemOrConsumable = item as (Item | Consumable) & { type: ItemType };
        return (
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <div>
                ...
                <span className="text-white font-semibold">{armor.base_score}</span>
              </div>
              <div>
                <span className="text-slate-400">Thresholds: </span>
                <span className="text-white font-semibold">{armor.base_thresholds}</span>
              </div>
            </div>
            {armor.feat_name && (
              <div className="rounded-lg bg-slate-600 p-2 mt-2">
                <div className="text-yellow-400 text-xs font-semibold mb-1">{armor.feat_name}</div>
                <div className="text-slate-300 text-xs">{armor.feat_text}</div>
              </div>
            )}
          </div>
        );
          ...
          <div className="space-y-2 text-sm">
            <div className="grid grid-cols-2 gap-4">
              <div className="flex justify-between">
                ...
                <span className="text-slate-300">{weapon.primary_or_secondary}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-slate-300">{weapon.damage.replace(/\s*(phy|mag)$/i, '')}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-slate-300">{weapon.physical_or_magical}</span>
              </div>
            </div>
            {weapon.feat_name && (
              <div className="rounded-lg bg-slate-600 p-2 mt-2">
                <div className="text-yellow-400 text-xs font-semibold mb-1">{weapon.feat_name}</div>
                <div className="text-slate-300 text-xs">{weapon.feat_text}</div>
              </div>
            )}
          </div>
        );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent 
        className="!w-[95vw] !max-w-[95vw] lg:!w-[80vw] lg:!max-w-[80vw] max-h-[80vh] overflow-hidden flex flex-col border-slate-700 bg-slate-800"
      >
        <DialogHeader>
          <DialogTitle className="text-white">Add Item to Inventory</DialogTitle>
          <DialogDescription className="text-slate-400">
            Search and select items from the SRD to add to your character&apos;s inventory.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-hidden flex flex-col space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2 flex-wrap lg:flex-nowrap">
                <span className="text-sm text-slate-400 self-center">Tier:</span>
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex items-start gap-4">
                      ...
                      <Button
                        size="sm"
                        className="flex-shrink-0 bg-sky-500 hover:bg-sky-600 text-white"
